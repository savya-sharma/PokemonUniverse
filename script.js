...
async function loadData() {
    ...
    const data = await getDataFromAPI("https://pokeapi.co/api/v2/pokemon?limit=20&offset=" + offset);
    offset += 20;
...
function showData(pokemons) {
    ...
    pokemons.forEach(getData => {
        ...
        mydiv.classList.add("parent", getData.name, getData.types[0].type.name);
        ...
        backpara.innerHTML =
            `<span>Height: ${getData.height}</span><br>
            <span>Weight: ${getData.weight}</span><br>
            <span>HP: ${getData.stats[0].base_stat}</span><br>
            <span>Attack: ${getData.stats[1].base_stat}</span><br>
            <span>Defense: ${getData.stats[2].base_stat}</span><br>
            <span>Special Attack: ${getData.stats[3].base_stat}</span><br>
            <span>Special Defense: ${getData.stats[4].base_stat}</span><br>
            <span>Speed: ${getData.stats[5].base_stat}</span>`;
